...
2) For every sample-app project
2.1) For every sample-app sub project
2.1.1) Check assembly folder
2.1.2) Find corresponding info in the projects.xml
2.1.3) Create and add new tags for resulting pom.xml
3) Open pom.xml
4) Find necessary places and insert new tags there

todo: http://stackoverflow.com/questions/229186/os-walk-without-digging-into-directories-below
todo: http://stackoverflow.com/questions/12931742/creating-xml-dynamically-with-lxml
todo: http://lxml.de/tutorial.html
...
def dependency_bundle(name):
    ...
    '''
    print('Creating new dependency tag for bundle=%s' % name)
...
def dependency(name, suffix):
    ...
    print('Creating new dependency tag for name=%s, suffix=%s' %
          (name, suffix))
...
def artifact_item(name, suffix):
    ...
    print('Creating new artifactItem tag for name=%s, suffix=%s' %
          (name, suffix))
...
def parse_sample_apps(path):
    ...
    print('Exclude: %s' % excludeSamples)
    ...
    for directory in os.listdir(path):
        ...
        if not os.path.isdir(dirpath):  # filter files
            print('Not directory, skip: %s' % dirpath)
            continue
        if directory[0] == '.':  # filter .idea, .git etc.
            print('Directory starts with ".", skip: %s' % dirpath)
            continue
            ...
            for assembly_file in os.listdir(assembly_files_dir):
                ...
                if ASSEMBLY_FILE_SEPARATOR in assembly_file:
                    ...
                    try:
                        ...
                        print('[ERROR] Wrong format: %s' % assembly_file_name)
                        continue

                    suffix = splitted[index_of_suffix]

                    print('Assembly demo (' + assembly_file_name + '): ' + suffix)
                    ...
                    print('[ERROR] No separator(\' - \') in the assembly file')

            print('Appending bundle dependency for %s' % name)
...
def insert_dependencies_into_pom(pom_tree, dependencies):
    ...
    dependencies_elements = pom_tree.xpath(
        '//*[contains(local-name(), "dependencies")]')
...
def insert_artifact_items_into_pom(pom_tree, artifact_items):
    print('Inserting artifact items')

    artifact_items_elements = pom_tree.xpath(
        '//*[contains(local-name(), "build")]' +
        '/*[contains(local-name(), "plugins")]' +
        '/*[contains(local-name(), "plugin")][*[contains(local-name(), "groupId")][text()="org.apache.maven.plugins"] and *[contains(local-name(), "artifactId")][text()="maven-dependency-plugin"]]' +
        '/*[contains(local-name(), "executions")]' +
        '/*[contains(local-name(), "execution")][*[contains(local-name(), "id")][text()="copy"]]' +
        '/*[contains(local-name(), "configuration")]' +
        '/*[contains(local-name(), "artifactItems")]')

    artifact_items_element = artifact_items_elements[0]
...
def generate_pom_xml(elements_tuple, source_pom_path):
    ...
    print('Parsing existing pom.xml')
    ...
    print('Starting to generate pom.xml')
...
def write_document(doc, target_pom_path):
    ...
    print('Writing document to the pom.xml')
...
def main(sample_apps_path, source_pom_path, target_pom_path):
    ...
    print('Script is started with args: sample_apps_path=%s, source_pom_path=%s, target_pom_path=%s' %
